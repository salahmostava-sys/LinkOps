...
export function TreasurySettingsTab() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<{ id: string; name: string; kind: 'account' | 'category' } | null>(null);
  ...
  return (
    <>
      <DeleteConfirmDialog
        ...
        title={deleteTarget?.kind === 'account' ? 'حذف الحساب' : 'حذف البند'}
        description={
          deleteTarget?.kind === 'account'
            ? `هل أنت متأكد من حذف الحساب "${deleteTarget?.name}"؟ لا يمكن التراجع عن هذا الإجراء.`
            : `هل أنت متأكد من حذف البند "${deleteTarget?.name}"؟ لا يمكن التراجع عن هذا الإجراء.`
        }
            ...
            {accounts.map(acc => (
              <div key={acc.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/40 border border-border/50 group">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => setDeleteTarget({ id: acc.id, name: acc.name, kind: 'account' })}
            ...
            {categories.map(cat => (
              <div key={cat.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/40 border border-border/50 group">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => setDeleteTarget({ id: cat.id, name: cat.name, kind: 'category' })}
                    className="opacity-0 group-hover:opacity-100 transition-opacity text-muted-foreground hover:text-destructive p-1 rounded"
                    title="حذف البند"
                    type="button"
                  >
