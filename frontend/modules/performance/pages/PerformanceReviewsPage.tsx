  ...
  return (
    <div className="space-y-5" dir="rtl">
      ...
      <div className="flex items-center gap-3">
        <button aria-label="الشهر السابق" onClick={() => setCurrentDate(d => subMonths(d, 1))} className="p-2 rounded-lg border border-border bg-card hover:bg-muted transition-colors"><ChevronRight size={16} /></button>
        <span className="text-base font-semibold text-foreground min-w-[160px] text-center">{toDisplayMonth(monthYear)}</span>
        <button aria-label="الشهر التالي" onClick={() => setCurrentDate(d => addMonths(d, 1))} className="p-2 rounded-lg border border-border bg-card hover:bg-muted transition-colors"><ChevronLeft size={16} /></button>
      </div>
