...
function statusTextColor(status: DocStatus): string {
  ...
  if (status === 'urgent') return 'text-orange-600';
  ...
  return (
    <div className="space-y-1">
      ...
      {daysLeft !== null && status !== 'ok' && status !== 'missing' && (
        <p className={`text-xs font-semibold ${statusTextColor(status)}`}>
...
function matchesFilterStatus(emp: EmployeeDoc, filterStatus: FilterStatus): boolean {
  if (filterStatus === 'all') return true;
  if (filterStatus === 'expired') return hasStatus(emp, 'expired');
  if (filterStatus === 'urgent') return hasStatus(emp, 'urgent');
  if (filterStatus === 'warning') return needsAttention(emp);
  return true;
}

function needsAttention(emp: EmployeeDoc): boolean {
  return DOC_KEYS.some(k => {
    const s = getDocStatus(emp[k]).status;
    return s === 'expired' || s === 'urgent' || s === 'warning';
  });
}

function hasStatus(emp: EmployeeDoc, target: DocStatus): boolean {
  return DOC_KEYS.some(k => getDocStatus(emp[k]).status === target);
}
...
const DocumentsPage = () => {
  ...
  const filtered = employees.filter(emp => {
    ...
    const matchStatus = matchesFilterStatus(emp, filterStatus);
