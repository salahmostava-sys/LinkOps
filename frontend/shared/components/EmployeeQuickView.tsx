...
async function loadEmployeeData(
  employeeId: string,
  setData: (data: EmployeeInfo | null) => void,
  ...
  try {
    ...
    const getStr = (val: unknown, fallback = '') => (typeof val === 'string' ? val : fallback);
    
    setData({
      id: getStr(e.id, employeeId),
      name: getStr(e.name, getStr(e.full_name)),
      phone: getStr(e.phone),
      city: getStr(e.city),
      tier: getStr(e.tier),
      work_type: getStr(e.work_type),
      join_date: getStr(e.join_date, getStr(e.created_at)),
      vehicle_name: getStr(e.vehicle_name, getStr(e.plate_number)),
      status: getStr(e.status, 'active'),
      employee_id: getStr(e.employee_id),
    });
  } catch {
    setData(null);
  } finally {
...
function EmployeeQuickViewDrawerBody({ loading, data }: Readonly<{ loading: boolean; data: EmployeeInfo | null }>) {
  if (loading) {
    return (
  ...
  return (
    <div className="space-y-4">
      ...
      <InfoRow
        ...
        value={data.join_date ? new Date(data.join_date).toLocaleDateString('ar-SA') : '—'}
      />
    </div>
  );
}
 ...
 */
export function EmployeeQuickView({ employeeId, open, onClose }: Readonly<Props>) {
  const [data, setData] = useState<EmployeeInfo | null>(null);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    if (open && employeeId) {
      loadEmployeeData(employeeId, setData, setLoading).catch(() => {});
    } else {
      setData(null);
    }
  }, [open, employeeId]);
    ...
    <>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          <EmployeeQuickViewDrawerBody loading={loading} data={data} />
        </div>
...
function InfoRow({ icon, label, value }: Readonly<{ icon: ReactNode; label: string; value: string }>) {
  return (
