...
function normalizeText(text: string): string {
  return text
    ...
    .replaceAll('ؤ', 'و') // توحيد الهمزة على الواو
    .replaceAll(/[َُِّْـ]/g, '') // إزالة التشكيل
    .replaceAll(/[^\u0600-\u06FFa-zA-Z0-9\s]/g, ''); // إزالة الرموز
}

function scoreWordOverlap(searchWords: string[], candidateWords: string[]): number {
  let matchedWords = 0;
...
function advancedSimilarity(searchName: string, candidateName: string): number {
  ...
  const matchedWords = scoreWordOverlap(searchWords, candidateWords);
