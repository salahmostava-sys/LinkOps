...
export const platformAccountService = {
  ...
  getAccounts: async () => {
    const { data, error } = await supabase.from('platform_accounts').select('id, app_id, employee_id, account_username, account_id_on_platform, iqama_number, iqama_expiry_date, status, notes, created_at').order('created_at', { ascending: false });
