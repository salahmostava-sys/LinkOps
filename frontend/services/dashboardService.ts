...
export const dashboardService = {
  ...
  fetchMainData: async (today: string, currentMonth: string, prevStart: string, prevEnd: string, sixDaysAgo: string) => {
    const [empRes, attRes, ordersRes, prevOrdersRes, weekAttRes, auditRes, empDetailsRes, vehiclesRes, alertsRes, appsRes, targetsRes, pricingRes] = await Promise.all([
      ...
      supabase.from('daily_orders').select('employee_id, app_id, orders_count, apps(id, name, brand_color, text_color), employees(name, city)').gte('date', currentMonth + '-01').lte('date', today).limit(5000),
      supabase.from('daily_orders').select('orders_count').gte('date', prevStart).lte('date', prevEnd).limit(5000),
  ...
  getComprehensiveStats: async (monthYear: string) => {
    ...
    const [
      ...
      supabase.from('maintenance_records').select('cost, status, vehicle_id').gte('date', start).lte('date', end),
      // Violations (pending)
      supabase.from('violations').select('amount, approval_status, employee_id').eq('approval_status', 'pending'),
      // Advances (active)
