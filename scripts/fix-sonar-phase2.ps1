...
foreach ($file in $tsFiles) {
    ...
    if ($content -ne $original) {
        ...
        $readonlyCount++
        Write-Output "Fixed readonly: $($file.Name)"
    }
}
...
foreach ($file in $tsFiles) {
    ...
    if ($content -ne $original) {
        Set-Content $file.FullName $content -NoNewline
        $findCount++
        Write-Output "Fixed find: $($file.Name)"
    }
}
...
foreach ($file in $tsFiles) {
    ...
    if ($content -ne $original) {
        Set-Content $file.FullName $content -NoNewline
        $voidCount++
        Write-Output "Fixed remaining void: $($file.Name)"
    }
}
Write-Output "Fixed $voidCount files with remaining void"

# --- Fix useAppColors.ts ---
# Already handled by Number.parseInt script

Write-Output "`n=== Phase 2 Script Done ==="
