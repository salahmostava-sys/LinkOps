...
function buildNamePattern(name) {
  const t = (name || '').trim();
  if (!t) return null;
  const escaped = t.replaceAll(/[\\%_]/g, String.raw`\$&`);
  return `%${escaped}%`;
}
...
async function callGroqChat(groqApiKey, groqBaseUrl, msgs, tools, model) {
  const selectedModel = model || process.env.GROQ_MODEL || 'llama3-8b-8192';
  const body = { model: selectedModel, messages: msgs, temperature: 0.3, max_tokens: 1024 };
  ...
  if (!groqRes.ok) {
    ...
    console.error(JSON.stringify({ level: 'error', scope: 'groq', status: groqRes.status, body: errText, ts: new Date().toISOString() }));
