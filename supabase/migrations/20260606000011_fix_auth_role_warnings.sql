...
CREATE POLICY "unified_delete_policy" ON public."salary_slip_templates" FOR DELETE
  USING ((((select auth.role()) = 'authenticated'::text) OR (is_active_user(( SELECT ( SELECT auth.uid() AS uid) AS uid)) AND (has_role(( SELECT ( SELECT auth.uid() AS uid) AS uid), 'admin'::app_role) OR has_role(( SELECT ( SELECT auth.uid() AS uid) AS uid), 'operations'::app_role)))));
...
CREATE POLICY "unified_insert_policy" ON public."salary_slip_templates" FOR INSERT
  WITH CHECK ((((select auth.role()) = 'authenticated'::text) OR (is_active_user(( SELECT ( SELECT auth.uid() AS uid) AS uid)) AND (has_role(( SELECT ( SELECT auth.uid() AS uid) AS uid), 'admin'::app_role) OR has_role(( SELECT ( SELECT auth.uid() AS uid) AS uid), 'operations'::app_role)))));
...
CREATE POLICY "unified_select_policy" ON public."salary_slip_templates" FOR SELECT
  USING ((((select auth.role()) = 'authenticated'::text) OR (is_active_user(( SELECT ( SELECT auth.uid() AS uid) AS uid)) AND (has_role(( SELECT ( SELECT auth.uid() AS uid) AS uid), 'admin'::app_role) OR has_role(( SELECT ( SELECT auth.uid() AS uid) AS uid), 'operations'::app_role))) OR (true OR is_active_user(( SELECT ( SELECT auth.uid() AS uid) AS uid)))));
