-- ============================================================
-- FIX: Dashboard Statistics & Data Integrity
-- ============================================================
-- 1. Modify v_rider_daily_platform_orders to use LEFT JOIN on apps
--    so that orders without a mapped app are not dropped.
-- 2. Modify performance_dashboard_rpc to include `totalEmployees` 
--    (count of all active employees) in the summary.
-- ============================================================
...
SELECT
  d.employee_id,
  COALESCE(e.name, 'غير معروف') AS employee_name,
  e.city,
  d.date,
  d.app_id,
  COALESCE(a.name, 'بدون تطبيق') AS app_name,
  COALESCE(a.brand_color, '#64748b') AS brand_color,
  SUM(d.orders_count)::INTEGER AS total_orders
FROM public.daily_orders AS d
JOIN public.employees AS e ON e.id = d.employee_id
LEFT JOIN public.apps AS a ON a.id = d.app_id
...
-- Recreate dependent views with security_invoker = true
CREATE OR REPLACE VIEW public.v_rider_daily_performance
WITH (security_invoker = true)
AS
SELECT
  p.employee_id,
...
CREATE OR REPLACE VIEW public.v_rider_monthly_performance
WITH (security_invoker = true)
AS
...
-- Re-create performance_dashboard_rpc to inject total_employees in the summary output
  ...
  RETURN (
    ...
    SELECT jsonb_build_object(
      'summary', jsonb_build_object(
        ...
        'totalEmployees', (SELECT COUNT(*) FROM public.employees WHERE active = true),
