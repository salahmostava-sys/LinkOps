...
--   Fix: ALTER FUNCTION ... SET search_path = public for each one.
--
...
-- ── Part 1: Fix search_path on all enum operator functions ───
-- Instead of ALTER-ing each function by hand, we re-create them
-- with SET search_path = public using the same loop pattern.
