-- ============================================================
-- SECURITY FIX v2 â€” ÙŠØ¹Ø§Ù„Ø¬ Ø¬Ù…ÙŠØ¹ Ø§Ù„ØªØ­Ø°ÙŠØ±Ø§Øª Ø§Ù„Ø£Ù…Ù†ÙŠØ© Ø§Ù„Ø¸Ø§Ù‡Ø±Ø© ÙÙŠ Supabase
--
...
--   3. anon_security_definer_function_executable  â†’ Ø¥Ù„ØºØ§Ø¡ ØµÙ„Ø§Ø­ÙŠØ© anon
--   4. authenticated_security_definer_function_executable â†’ Ø¥Ù„ØºØ§Ø¡ ØµÙ„Ø§Ø­ÙŠØ© authenticated Ù„Ù„Ø¯ÙˆØ§Ù„ Ø§Ù„Ø¯Ø§Ø®Ù„ÙŠØ©
-- ============================================================
...
--    Ø¥Ø¹Ø§Ø¯Ø© Ø¥Ù†Ø´Ø§Ø¡ Ø§Ù„Ø¯Ø§Ù„Ø© Ù…Ø¹ SET search_path = '' Ù„Ù…Ù†Ø¹ Ù‡Ø¬Ù…Ø§Øª Ø­Ù‚Ù† search_path
-- ============================================================
...
-- ============================================================
-- 4. Ø¥Ù„ØºØ§Ø¡ ØµÙ„Ø§Ø­ÙŠØ© anon Ø¹Ù„Ù‰ Ø¬Ù…ÙŠØ¹ Ø¯ÙˆØ§Ù„ SECURITY DEFINER
--    (Ø´Ø§Ù…Ù„Ø© Ù„Ù„Ø¯ÙˆØ§Ù„ Ø§Ù„ØªÙŠ Ù‚Ø¯ Ù„Ø§ ÙŠÙƒÙˆÙ† Ù‚Ø¯ Ø´Ù…Ù„Ù‡Ø§ Ø§Ù„Ù€ migration Ø§Ù„Ø³Ø§Ø¨Ù‚)
-- ============================================================

-- â”€â”€ Ø§Ù„Ø±ÙˆØ§ØªØ¨ / Ø§Ù„Ù…Ø¯ÙÙˆØ¹Ø§Øª â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
-- â”€â”€ Ø§Ù„Ù…ÙˆØ¸ÙÙˆÙ† / Ø§Ù„Ø­Ø¶ÙˆØ± â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
-- â”€â”€ Ù„ÙˆØ­Ø© Ø§Ù„ØªØ­ÙƒÙ… / Ø§Ù„ØªÙ‚Ø§Ø±ÙŠØ± â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
-- ============================================================
-- 5. Ø¥Ù„ØºØ§Ø¡ ØµÙ„Ø§Ø­ÙŠØ© authenticated Ø¹Ù„Ù‰ Ø§Ù„Ø¯ÙˆØ§Ù„ Ø§Ù„Ø¯Ø§Ø®Ù„ÙŠØ© ÙÙ‚Ø·
--    (Ø§Ù„Ø¯ÙˆØ§Ù„ Ø§Ù„ØªÙŠ ØªØ³ØªØ¯Ø¹ÙŠÙ‡Ø§ Ø§Ù„Ù€ triggers Ø£Ùˆ Ù…Ø³Ø§Ø¹Ø¯Ø§Øª Ø¯Ø§Ø®Ù„ÙŠØ© ÙÙ‚Ø·ØŒ
...
-- Ø¯ÙˆØ§Ù„ Ø§Ø®ØªØ¨Ø§Ø± / ØªØµØ­ÙŠØ­
REVOKE EXECUTE ON FUNCTION public.test_shift_salary()      FROM authenticated;

-- Ù…Ø³Ø§Ø¹Ø¯ Ø¯Ø§Ø®Ù„ÙŠ ÙŠØ³ØªØ¯Ø¹ÙŠÙ‡ plpgsql ÙÙ‚Ø· â€” Ù„Ø§ ÙŠÙØ³ØªØ¯Ø¹Ù‰ Ù…Ø¨Ø§Ø´Ø±Ø© Ø¹Ø¨Ø± supabase.rpc()
REVOKE EXECUTE ON FUNCTION public.calculate_order_salary_for_app(uuid, integer, integer, uuid[], boolean) FROM authenticated;

-- Ù†Ø³Ø® dashboard_overview Ø§Ù„Ù‚Ø¯ÙŠÙ…Ø© (legacy) â€” Ø§Ù„ØªØ·Ø¨ÙŠÙ‚ ÙŠØ³ØªØ®Ø¯Ù… dashboard_overview_rpc ÙÙ‚Ø·
