...
CREATE POLICY "Finance/admin can manage advances"
  ...
  USING (is_active_user(auth.uid()) AND (has_role(auth.uid(), _const_role_admin()) OR has_role(auth.uid(), _const_role_finance())));
...
CREATE POLICY "Operations/admin can manage daily_orders"
  ...
  USING (is_active_user(auth.uid()) AND (has_role(auth.uid(), _const_role_admin()) OR has_role(auth.uid(), _const_role_operations()) OR has_role(auth.uid(), _const_role_hr())));
